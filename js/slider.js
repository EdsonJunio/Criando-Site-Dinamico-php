$(function(){
   ...
   function initSlider(){
      ...
      for(var i = 0; i < maxSlide+1; i++){
         ...
         if(i == 0)
         content+='<span class="ective-slidere"></span>';
         else
         content+='<span></span>';
   ...
   function changeSlide(){
       setInterval(function(){
        ...
        curSlide = 0;
          ...
          // troca bullets da navegaçao do slider!
...
$('body').on('click','.bullets span' ,function(){
   var currentBullert = $(this);
   $('.banner-single').eq(curSlide).stop().fadeIn(1000);
   curSlide = currentBullert.index();
   $('.banner-single').eq(curSlide).stop().fadeIn(1000);
   $('.bullets span').removeClass('active-slider');
   currentBullert.addClass('active-slider');
} );
